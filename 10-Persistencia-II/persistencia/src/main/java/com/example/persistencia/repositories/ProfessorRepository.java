...
public class ProfessorRepository {
    ...
    public List<Professor> obterProfessores() {
        ...
        try {
            ...
            resultados = consulta.executeQuery("SELECT p.*, c.nome AS curso_nome FROM professores p JOIN cursos as c ON p.curso_id = c.curso_id;");
            ...
            while (resultados.next()) { // Itera a tabela de resultados
                ...
                // Curso curso = new Curso(); // Para cada linha lida um novo curso é criado(Forma Incorreta)
    ...
    public Professor inserir(Professor professor){
        ...
        try{
            String comando = "INSERT INTO professores(nome, email, data_nascimento, salario_base, curso_id) VALUES(?, ?, ?, ?, ?)";
            ...
            int linhasAfetas = consulta.executeUpdate(comando);

            if(linhasAfetas > 0){
                System.out.println("SUCESSO: " + linhasAfetas + "linhas afetadas!");
                ...
                System.out.println("AVISO: " + linhasAfetas + "inserções");
            }
        }
        catch( SQLException e){
