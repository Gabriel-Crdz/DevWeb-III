...
public class ProfessorRepository {
    ...
    public Professor resultadoProfessor(ResultSet resultados, Curso curso) throws SQLException{
        ...
        p.setSenhaHashed(resultados.getString("senha")); // Salvando a senha já criptografada
    ...
    public Professor findProfessorById(Integer cod) {
     ...
     public Professor findProfessorByEmail(String email) {
        ...
        try {
            ...
            /* Tratamento do SQL */
            String comando = "SELECT p.*, c.nome AS curso_nome"
            + " FROM professores p JOIN cursos as c ON p.curso_id = c.curso_id WHERE p.email = ? ;";
            consulta = conexao.prepareStatement(comando); // Consulta preparada

            consulta.setString(1, email); // Passando o valor do parametro para o comando sql (segurança contra SQL Injection)
    ...
    public Professor inserir(Professor professor){
        ...
        try{
            String comando = "INSERT INTO professores (nome, email, data_nascimento, salario_base, curso_id, senha) " +
            "VALUES(?, ?, ?, ?, ?, ?)";
